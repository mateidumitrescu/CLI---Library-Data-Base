...
void ll_add_nth_node(linked_list_t* list, unsigned int n, void *new_data, void *details)
{
    ll_node_t *prev, *curr;
    ll_node_t* new_node;
		               ...
		               int (*compare_function)(void*, void*), char dictionary_type[S])
{
    ...
    for (unsigned int i = 0; i < ht->hmax; i++) {
        if (strcmp(dictionary_type, BOOKS) == 0) {
            ht->buckets[i] = ll_create(sizeof(key_value_t), sizeof(book_info_t));
            ...
            ht->buckets[i] = ll_create(sizeof(key_value_t), sizeof(user_info_t));
        }
...
void ht_remove_entry(hashtable_t *ht, void *key, char dictionary_type[S])
{
    ...
    data = (key_value_t *)current->data;
    if (strcmp(dictionary_type, BOOKS) == 0) {  // we have to check what type of dictionary we work with
        hashtable_t *definitions_ht = (hashtable_t *)data->value;
